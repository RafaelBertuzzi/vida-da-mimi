...
function geronimo() {
	...
	function Game() {
		...
		this.ghostSpeedNormal = (this.level > 1 ? 3 : 2); // global default for ghost speed
		...
		this.nextLevel = function () {
			...
			if (this.level === FINAL_LEVEL) {
				...
				game.pauseAndShowMessage("Nível " + game.level, this.getLevelTitle() + "<br/>(Aperte para continuar)");
		...
		this.getLevelTitle = function () {
			switch (this.level) {
				case 2:
					return 'Tranquilo, né? Tipo ficar no Instagram o dia inteiro.';
				case FINAL_LEVEL:
					return 'Foi tão difícil quanto<br/>trabalhar no supermercado da família.';
				default:
					return '"nada novo"';
			}
		}
		...
		this.showHighscoreForm = function () {
			this.pauseAndShowMessage("Fim de Jogo", "Aposto que foi tão difícil quanto<br/>abrir os olhos de manhã sendo rica.");
		}

		this.showEndOfLivesMessage = function () {
			this.pauseAndShowMessage("Você perdeu!", "Mimi, mesmo que você falhe,<br/>a sua vida sempre vai ser fácil.");
		}
		...
		this.forcePause = function () {
			this.timer.stop();
			this.pauseAndShowMessage("Já ta achando difícil, Mimi?", "Aperte para continuar");
		}
		...
		this.init = async (state) => {
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost(GHOSTS.PINKY, 7, 5, 'img/ansiedade.svg', 2, 2);
				inky = new Ghost(GHOSTS.INKY, 8, 5, 'img/boleto.svg', 13, 11);
				blinky = new Ghost(GHOSTS.BLINKY, 9, 5, 'img/trabalho.svg', 13, 0);
				clyde = new Ghost(GHOSTS.CLYDE, 10, 5, 'img/merito.svg', 2, 11);
			} else {
				pinky.reset();
				inky.reset();
				blinky.reset();
		...
		this.move = function () {
			...
			if (this.ghostHouse == true) {
				...
				if (this.name == GHOSTS.CLYDE) {
					if ((game.level < 3) || ((game.pillCount > 104 - 20))) this.stop = true;
					else this.stop = false;
				}
				if (this.name == GHOSTS.INKY) {
					if ((game.level < 2) || ((game.pillCount > 104 - 10))) this.stop = true;
					else this.stop = false;
				}
